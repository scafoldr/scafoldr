      ...
      <ResizableLayout
        leftPanel={
          <div className="h-full flex flex-col relative">
            {/* DBML Editor - takes remaining space */}
            <div
              className={`
                border border-slate-200 dark:border-slate-800 rounded-md overflow-hidden
                transition-all duration-300 ease-in-out
                ${showAiChat ? 'h-[60%]' : 'h-full'}
              `}>
              <Code selectedFile={dbmlFile} onFileEdit={handleDbmlCodeChange} />
            </div>

            {/* AI Chat Panel - fixed height at bottom */}
            {showAiChat && (
              <div
                className="absolute bottom-0 left-0 right-0 h-[40%]
                  ...
                  flex flex-col
                  overflow-hidden">
                <div className="flex items-center justify-between p-2 border-b border-slate-200 dark:border-slate-800 flex-shrink-0">
            ...
            {/* AI Chat Button - absolute positioned */}
            {!showAiChat && (
